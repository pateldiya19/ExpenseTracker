...
export default function TopBar({ onSidebarToggle, sidebarOpen }) {
  ...
  useEffect(() => {
		const handler = (e) => {
			const { totalSpending, limitAmount } = e.detail || {};
			toast({ title: 'Spending Limit Exceeded', description: `You've spent ₹${(totalSpending/100).toFixed(2)} this month (limit ₹${(limitAmount/100).toFixed(2)}).` });
		};
		window.addEventListener('notifications:spending_limit_exceeded', handler);
		return () => window.removeEventListener('notifications:spending_limit_exceeded', handler);
	}, []);
